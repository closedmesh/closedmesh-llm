...
/// Wait for first response byte from QUIC (10s timeout). If it arrives, write
/// it to the TCP stream and return the QUIC recv stream for continued relay.
/// If it fails, the TCP stream is untouched (no bytes written) so caller can retry.
pub async fn relay_quic_first_byte(
    mut quic_recv: iroh::endpoint::RecvStream,
    ...
    let first_n = match first_read {
        Err(_) => anyhow::bail!("no response within 10s"),
        Ok(Ok(Some(n))) if n > 0 => n,
        Ok(Ok(_)) => anyhow::bail!("empty response from host"),
        Ok(Err(e)) => return Err(e.into()),
    };

    // Got first bytes — commit: write to TCP client
    tcp_stream.write_all(&buf[..first_n]).await?;
    BYTES_TRANSFERRED.fetch_add(first_n as u64, Ordering::Relaxed);

    Ok(quic_recv)
}

/// Relay the remainder of a response after first bytes have been committed.
/// Handles both directions: client→QUIC (for keep-alive/close) and QUIC→client.
pub async fn relay_remainder(
    _tcp_read: tokio::io::ReadHalf<TcpStream>,
    mut tcp_write: tokio::io::WriteHalf<TcpStream>,
    _quic_send: iroh::endpoint::SendStream,
    mut quic_recv: iroh::endpoint::RecvStream,
) -> Result<()> {
    // For HTTP request/response over QUIC: the full request was already sent,
    // just relay the response back to the client.
    let mut buf = vec![0u8; 64 * 1024];
    loop {
        match quic_recv.read(&mut buf).await {
            Ok(Some(n)) => {
                if let Err(e) = tcp_write.write_all(&buf[..n]).await {
                    tracing::debug!("relay_remainder: client write error: {e}");
                    break;
                }
                BYTES_TRANSFERRED.fetch_add(n as u64, Ordering::Relaxed);
            }
            Ok(None) => break,
            Err(e) => {
                tracing::debug!("relay_remainder: QUIC read error: {e}");
                break;
            }
        }
    }
