...
Roles are exchanged via gossip. Live-state badges are separate and use `Client`, `Standby`, `Loading`, and `Serving`. Preferred peers use `meshllm.node.v1` protobuf on QUIC ALPN `closedmesh/1`; legacy peers may still negotiate `closedmesh/0` and use the older JSON gossip payloads. A node transitions Worker → Host when elected.
...
The control plane uses QUIC ALPN `closedmesh/1` with the `meshllm.node.v1` protobuf schema. Scoped control-plane streams use 4-byte LE framing followed by protobuf bytes.
...
- **Unnamed mesh**: random UUID, persisted to `~/.closedmesh/mesh-id`
...
Saved to `~/.closedmesh/last-mesh` on successful join for sticky preference scoring.
...
This gives instant API access (within seconds of `closedmesh serve --join`) while the local
GPU loads its model in the background.

## Local Node Config

`closedmesh serve` owns startup model configuration. By default it reads
`~/.closedmesh/config.toml`, which now serves as the unified local node config for:
...
- valid IDs come from the local `closedmesh gpus` / `closedmesh gpus --json` inventory surface
...
Bare `closedmesh serve` is the config-owned path. If `[[models]]` is empty, it warns,
...
| `/api/search` | GET | Search the built-in catalog or Hugging Face with the same JSON payload shape as `closedmesh models search --json` |
...
provides the same read-only model search payload as `closedmesh models search --json`
...
`closedmesh` with no arguments prints the standard CLI help and exits.
