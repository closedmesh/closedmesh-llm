            ...
            >
              ...
              {apiDirectUrl ? (
                ...
                <div className="space-y-2">
                  <div className="text-xs text-muted-foreground">
                    Run closedmesh locally to get an OpenAI-compatible API on your
                    machine:
                  </div>
                  <div className="flex items-center gap-2 rounded-md border bg-muted/40 px-2 py-1.5">
                    <code className="min-w-0 flex-1 overflow-x-auto whitespace-nowrap text-xs">
                      {isPublicMesh
                        ? "closedmesh --auto"
                        : `closedmesh --auto --join ${inviteToken || "(token)"}`}
                    </code>
                    <Button
                      type="button"
                      size="icon"
                      ...
                      onClick={() =>
                        void navigator.clipboard.writeText(
                          isPublicMesh
                            ? "closedmesh --auto"
                            : `closedmesh --auto --join ${inviteToken || ""}`,
                        )
                      }
                    >
                ...
                <div className="space-y-1">
                  {agentLaunchers.map((agent) => {
                    const cmd = isPublicMesh
                      ? `closedmesh ${agent}`
                      : `closedmesh ${agent} --join ${inviteToken || "(token)"}`;
                    return (
                      <div
                        key={agent}
