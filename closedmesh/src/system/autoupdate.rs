...
const DEFAULT_RELEASE_REPO: &str = "Mesh-LLM/closedmesh";
#[cfg(not(windows))]
const INSTALL_SCRIPT_URL: &str =
    "https://raw.githubusercontent.com/Mesh-LLM/closedmesh/main/install.sh";
const RELEASES_URL: &str = "https://github.com/Mesh-LLM/closedmesh/releases/latest";
const SELF_UPDATE_ATTEMPTED_ENV: &str = "CLOSEDMESH_SELF_UPDATE_ATTEMPTED";
const SELF_UPDATE_REPO_ENV: &str = "CLOSEDMESH_SELF_UPDATE_REPO";
    ...
    if let Some(release) = latest_release_info().await {
        ...
        match bundle_asset {
            Some(ref asset) if release.assets.iter().any(|a| a == asset) => {
                eprintln!(
                    "✨ New version: v{VERSION} -> v{}. Run 'closedmesh update'.",
                    release.version
                );
            }
            _ => {
...
pub(crate) async fn run_update_command(cli: &Cli) -> Result<()> {
    ...
    if requested_version.is_none() && !version_newer(&release.version, VERSION) {
        eprintln!("closedmesh is already up to date (v{VERSION}).");
        return Ok(());
    }
...
fn require_update_target(cli: &Cli) -> Result<UpdateTarget> {
    if !platform_has_release_assets() {
        bail!(
            "`closedmesh update` is not supported on this platform. Download the latest release from {RELEASES_URL}."
        );
    }

    let exe = std::env::current_exe().context("Cannot determine closedmesh executable path")?;
    let Some((install_dir, bundle_flavor)) = bundle_install_dir(&exe, cli.llama_flavor) else {
        bail!(
            "`closedmesh update` only works for release-bundle installs. Current executable: {}",
            exe.display()
        );
    };
...
async fn apply_update_if_available(
    ...
    eprintln!(
        "⬇️ Updating closedmesh v{VERSION} -> v{} ({})...",
        release.version,
        target.bundle_flavor.suffix()
    );
    match install_latest_bundle(
...
fn mesh_binary_name() -> String {
    launch::platform_bin_name("closedmesh")
}
...
mod zip_tests {
    ...
    fn extract_zip_archive_strips_top_level_directory() -> Result<()> {
        let base_dir = unique_temp_dir("closedmesh_extract_zip_test");
...
try {{
    ...
    if ($restartAfterUpdate) {{
        $env:CLOSEDMESH_SELF_UPDATE_ATTEMPTED = '1'
        & $exePath @args
        exit $LASTEXITCODE
    }}
    exit 0
...
fn exec_current_binary(exe: &Path) -> Result<()> {
    ...
    Err(errno).context("Failed to restart updated closedmesh")
}
...
mod tests {
    ...
    fn test_release_asset_url() {
        ...
        assert_eq!(
            ...
            "https://github.com/Mesh-LLM/closedmesh/releases/download/v0.60.0/closedmesh-darwin-aarch64.tar.gz"
        );
    }
    ...
    fn test_release_repo_defaults_to_main_repo() {
        std::env::remove_var(SELF_UPDATE_REPO_ENV);
        assert_eq!(release_repo(), "Mesh-LLM/closedmesh");
        assert_eq!(
            latest_release_api_url(),
            "https://api.github.com/repos/Mesh-LLM/closedmesh/releases/latest"
        );
    }
    ...
    fn test_release_repo_can_be_overridden_for_testing() {
        std::env::set_var(SELF_UPDATE_REPO_ENV, "jdumay/closedmesh");
        assert_eq!(release_repo(), "jdumay/closedmesh");
        assert_eq!(
            latest_release_api_url(),
            "https://api.github.com/repos/jdumay/closedmesh/releases/latest"
        );
        assert_eq!(
            release_api_url_for_tag("v0.60.0"),
            "https://api.github.com/repos/jdumay/closedmesh/releases/tags/v0.60.0"
        );
        assert_eq!(
            release_asset_url("v0.60.0", "closedmesh-linux-x86_64.tar.gz"),
            "https://github.com/jdumay/closedmesh/releases/download/v0.60.0/closedmesh-linux-x86_64.tar.gz"
        );
        std::env::remove_var(SELF_UPDATE_REPO_ENV);
    }
    ...
    fn test_should_attempt_auto_update_only_when_flag_is_set() {
        ...
        let cli = Cli::parse_from(["closedmesh", "--auto-update", "--auto"]);
        assert!(should_attempt_auto_update(&cli));

        let cli = Cli::parse_from(["closedmesh", "download"]);
        assert!(!should_attempt_auto_update(&cli));

        let cli = Cli::parse_from(["closedmesh", "--auto-update", "update"]);
        assert!(!should_attempt_auto_update(&cli));
    }
