...
/// `~/.cache/closedmesh/benchmark-fingerprint.json`.
...
mod tests {
    ...
    fn test_detect_benchmark_binary_linux_cuda_finds_crate_release_helper() {
        ...
        let crate_release = root.join("closedmesh/target/release");
    ...
    fn test_detect_benchmark_binary_macos_soc_finds_crate_release_helper() {
        let root = std::env::temp_dir().join(format!(
            "closedmesh-benchmark-lookup-macos-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        let bin_dir = root.join("llama.cpp/build/bin");
    ...
    fn test_fingerprint_cache_roundtrip() {
        let path = std::env::temp_dir().join("closedmesh-test-fingerprint-roundtrip.json");
        let fp = make_fingerprint(
            vec![GpuBandwidth {
                name: "NVIDIA A100".into(),
                vram_bytes: 80_000_000_000,
    ...
    fn test_try_save_fingerprint_overwrites_existing_cache() {
        let path = std::env::temp_dir().join("closedmesh-test-fingerprint-overwrite.json");
    ...
    fn test_run_and_save_missing_binary_fails_cleanly() {
        let root = std::env::temp_dir().join(format!(
            "closedmesh-run-and-save-missing-{}",
            std::process::id()
        ));
        let bin_dir = root.join("bin");
        let path = root.join("benchmark-fingerprint.json");
    ...
    fn test_old_cache_format_fails_parse() {
        ...
        let path = std::env::temp_dir().join("closedmesh-test-fingerprint-old-format.json");
    ...
    fn test_old_fingerprint_cache_loads_without_tflops() {
        ...
        let path = std::env::temp_dir().join("closedmesh-test-fingerprint-pre-tops.json");
    ...
    fn test_fingerprint_path_filename() {
        ...
        assert!(
            parent.ends_with("closedmesh"),
            "fingerprint should be under closedmesh cache directory, got {:?}",
            parent
        );
    }
    ...
    fn test_run_benchmark_kills_on_timeout() {
        ...
        let dir = std::env::temp_dir().join("closedmesh-test-bm-timeout");
