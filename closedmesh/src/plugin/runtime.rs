...
impl ExternalPlugin {
    ...
    async fn ensure_running(&self) -> Result<()> {
        ...
        child.env("CLOSEDMESH_PLUGIN_ENDPOINT", &endpoint);
        child.env("CLOSEDMESH_PLUGIN_TRANSPORT", transport);
        child.env("CLOSEDMESH_PLUGIN_NAME", &self.spec.name);
        if let Some(ref url) = self.spec.url {
            child.env("CLOSEDMESH_OPENAI_ENDPOINT_URL", url);
        }
