...
async fn start_blobstore_plugin_manager_for(
    ...
    manifests.insert(
        ...
        closedmesh_plugin::proto::PluginManifest {
            capabilities,
            ..Default::default()
        },
    );
...
async fn test_api_proxy_lemonade_integration_when_enabled() {
    if std::env::var("CLOSEDMESH_TEST_LEMONADE").ok().as_deref() != Some("1") {
        return;
    }
    ...
    let models_response = client
        ...
        .send()
        .await
        .expect("Lemonade should be reachable when CLOSEDMESH_TEST_LEMONADE=1")
        .error_for_status()
