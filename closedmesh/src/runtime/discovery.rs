...
/// Ensure closedmesh is running on `port`, then return (available_models, chosen_model, spawned_child).
///
/// Launcher behavior: if nothing is listening yet, auto-start `closedmesh client --auto`
...
pub(crate) async fn check_mesh(
    ...
    if client.get(&url).send().await.is_err() {
        let _ = emit_event(OutputEvent::Info {
            message: format!("No closedmesh on port {port} — starting background auto-join node"),
            context: None,
        });
        let exe = std::env::current_exe().unwrap_or_else(|_| "closedmesh".into());
        child = Some(
    ...
    if models.is_empty() {
        ...
        anyhow::bail!(
            "closedmesh on port {port} has no models yet (or could not be reached).\n\
