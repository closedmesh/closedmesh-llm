...
fn pinned_gpu_config_peer_error(peer_version: Option<&str>) -> String {
    ...
    format!(
        "pinned gpu config sync requires closedmesh >= {MIN_PINNED_GPU_CONFIG_PEER_VERSION}; subscriber advertised {advertised}"
    )
}
...
impl Node {
    ...
    pub async fn start(
...

        {
            ...
            let urls: Vec<String> = if relay_urls.is_empty() {
                ...
                // (*.michaelneale.closedmesh.iroh.link) silently rotted off
    ...
    pub(crate) async fn subscribe_to_config(
        ...
        let req = ConfigSubscribe {
            ...
            // Owner-id filtering is an embedded-client concept; closedmesh does
...
/// Named meshes: `sha256("closedmesh:" + name + ":" + nostr_pubkey)` — deterministic, unique per creator.
