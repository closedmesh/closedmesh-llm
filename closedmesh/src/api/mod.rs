...
//!   GET  /api/search    — catalog or Hugging Face model search with the same JSON payload as `closedmesh models search --json`
...
const CLOSEDMESH_VERSION: &str = crate::VERSION;
        ...
        let local_instances: Vec<LocalInstance> = {
            ...
            if instances.is_empty() {
                instances.push(LocalInstance {
                    ...
                    version: Some(CLOSEDMESH_VERSION.to_string()),
...
mod tests {
    ...
    fn temp_blobstore_root(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("closedmesh-api-{name}-{}", rand::random::<u64>()))
    }
    ...
    async fn test_api_search_catalog_returns_canonical_model_refs() {
        ...
        assert_eq!(
            hit["show"],
            json!("closedmesh models show Qwen3-Coder-Next-Q4_K_M")
        );
...
"#
    ...
    async fn status_payload_safety_net_adds_self_when_empty() {
        ...
        assert_eq!(instances[0].version, Some(CLOSEDMESH_VERSION.to_string()));
    }
