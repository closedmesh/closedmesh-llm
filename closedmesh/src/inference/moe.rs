...
fn mesh_cache_dir() -> PathBuf {
    if let Ok(path) = std::env::var("XDG_CACHE_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("closedmesh");
        }
    }
...
/// Stored under the closedmesh cache root so local sidecar data never pollutes the model directory.
...
fn write_shared_ranking_artifact(
    ...
    let mut lines = vec![
        "# closedmesh-moe-ranking=v1".to_string(),
...
mod tests {
    ...
    fn split_path_uses_mesh_cache_root() {
        ...
        std::env::set_var("XDG_CACHE_HOME", "/tmp/closedmesh-cache-root");
        ...
        assert_eq!(
            path,
            PathBuf::from("/tmp/closedmesh-cache-root")
                .join("closedmesh")
                .join("splits")
    ...
    fn split_path_migrates_legacy_split_if_present() {
        let prev_xdg = std::env::var_os("XDG_CACHE_HOME");
        let base =
            std::env::temp_dir().join(format!("closedmesh-moe-migrate-{}", std::process::id()));
    ...
    fn split_path_migrates_legacy_split_from_unstripped_multifile_stem() {
        ...
        let base = std::env::temp_dir().join(format!(
            "closedmesh-moe-migrate-multifile-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&base);
        let cache_root = base.join("cache");
