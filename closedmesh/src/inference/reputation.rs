...
//! The score survives restarts via `~/.closedmesh/reputation.json`.
//!
...
//! (`CLOSEDMESH_VERIFY_ENFORCE`). Reputation is the slow, accumulated companion
...
/// Path of the reputation store file (`~/.closedmesh/reputation.json`). Honors
/// `CLOSEDMESH_HOME` / `HOME` the same way the native-baseline cache does.
pub fn store_path() -> Option<PathBuf> {
    if let Ok(custom) = std::env::var("CLOSEDMESH_HOME") {
        return Some(PathBuf::from(custom).join(STORE_FILENAME));
    }
    let home = dirs::home_dir()?;
    Some(home.join(".closedmesh").join(STORE_FILENAME))
}
