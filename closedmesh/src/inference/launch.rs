        ...
        match self {
            ...
            KvCacheWarning::QuantizedVBreaksMetalFaFallback => {
                "Known upstream bug: Metal crashes on quantized V cache when Flash \
                 ...
                 file a closedmesh bug — it should not happen. Older Intel Macs or any \
...
pub async fn start_rpc_server(
    ...
    command
        .args(&args)
        .env("CLOSEDMESH_OWNER_PID", std::process::id().to_string())
        .env(
            "CLOSEDMESH_RUNTIME_DIR",
...
pub async fn start_llama_server(
    ...
    //     pointing closedmesh at an external llama.cpp binary can still trip
...
"#;
    ...
    async fn terminate_unsafe_pid_returns_false() {
        let result = terminate_process(1, "closedmesh", None).await;
    ...
    fn log_path_does_not_duplicate_extension() {
        ...
        let original_root = env::var("CLOSEDMESH_RUNTIME_ROOT").ok();
        env::set_var("CLOSEDMESH_RUNTIME_ROOT", temp_path);
        ...
        if let Some(orig) = original_root {
            env::set_var("CLOSEDMESH_RUNTIME_ROOT", orig);
        } else {
            env::remove_var("CLOSEDMESH_RUNTIME_ROOT");
        }
    }
