...
//!
//! A running mesh publishes a replaceable event (kind 31990, d-tag "closedmesh")
...
impl Publisher {
    ...
    pub async fn publish(&self, listing: &MeshListing, ttl_secs: u64) -> Result<()> {
        ...
        let tags = vec![
            Tag::custom(TagKind::Custom("d".into()), vec!["closedmesh".to_string()]),
            Tag::custom(TagKind::Custom("k".into()), vec!["closedmesh".to_string()]),
            Tag::custom(
...
///   - the blessed community name "closedmesh".
///
...
pub fn is_auto_eligible(mesh: &DiscoveredMesh) -> bool {
    match mesh.listing.name.as_deref() {
        None => true,
        Some(name) => name.eq_ignore_ascii_case("closedmesh"),
    }
}
...
pub fn score_mesh(mesh: &DiscoveredMesh, _now_secs: u64, last_mesh_id: Option<&str>) -> i64 {
    ...
    // anything else in `--auto`. The literal name "closedmesh" is treated as a
    ...
    match mesh.listing.name.as_deref() {
        None => score += 300,
        Some(n) if n.eq_ignore_ascii_case("closedmesh") => score += 300,
        Some(_) => {}
    }
...
pub fn smart_auto_with_self(
    ...
    // plus the blessed name "closedmesh". Other named meshes are still publicly
...
mod scoring_tests {
    ...
    #[test]
    fn score_closedmesh_alias_matches_unnamed() {
        // "closedmesh" is a defensive alias for the community mesh and must
...
mod smart_auto_tests {
    ...
    fn smart_auto_both_community_aliases_eligible() {
        // Both unnamed listings and the "closedmesh" alias are eligible for
        ...
        // candidates and that `"closedmesh"` is no longer penalised relative
        // to unnamed.
        let meshes = vec![
        ...
        assert_eq!(
            ...
            "None and 'closedmesh' should score equally as community aliases",
        );
    ...
    fn smart_auto_excludes_other_named_meshes() {
        ...
        // (unnamed or name == "closedmesh"). Other named meshes — even though
    ...
    fn smart_auto_larger_unnamed_beats_smaller_alias() {
        // Both unnamed and "closedmesh" are eligible with the same name bonus.
...
mod key_file_tests {
    ...
    fn load_or_create_keys_at_round_trips() {
        let path = temp_key_path("closedmesh-nostr-key");
    ...
    fn load_or_create_keys_at_hardens_existing_permissions() {
        ...
        let path = temp_key_path("closedmesh-nostr-key-perms");
    ...
    fn load_or_create_keys_at_rejects_symlink_key() {
        ...
        let path = temp_key_path("closedmesh-nostr-key-symlink");
        let dir = path.parent().unwrap();
        std::fs::create_dir_all(dir).unwrap();
...
mod integration_tests {
    ...
    async fn publish_discover_round_trip() {
        ...
        let mesh_name = format!("closedmesh-test-{}", rand::random::<u32>());
