...
//! `~/.closedmesh/serving-tally.json` so the count survives the silent
...
pub struct ServingTally {
    ...
    /// so they never touch `~/.closedmesh`).
    path: Option<PathBuf>,
...
/// Path of the tally file (`~/.closedmesh/serving-tally.json`). Honors
/// `CLOSEDMESH_HOME` / `HOME` exactly like the native-baseline cache.
pub fn tally_path() -> Option<PathBuf> {
    ...
    Some(home.join(".closedmesh").join(TALLY_FILENAME))
}
