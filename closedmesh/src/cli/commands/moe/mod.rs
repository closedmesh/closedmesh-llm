...
fn print_submit_suggestion(model_path: &Path) {
    ...
    println!("📤 Contribute this ranking to closedmesh so other users can reuse it:");
    println!("  closedmesh moe share '{}'", identity.distribution_ref());
}
...
async fn run_share_resolved(
    ...
    let token = models::hf_token_override().ok_or_else(|| {
        share_error(
            ...
            "Set HF_TOKEN or HUGGING_FACE_HUB_TOKEN before running `closedmesh moe share`.",
        )
    })?;
