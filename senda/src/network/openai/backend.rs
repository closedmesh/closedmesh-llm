...
async fn handle_connection(mut stream: TcpStream, llama_port: u16, node: Node) -> Result<()> {
    ...
    // INFO/WARN for the `senda` crate, so anything higher than DEBUG
    ...
    // To re-enable in production, set `RUST_LOG=senda::network::openai::backend=debug`.
    tracing::debug!(
        path = %request.path,
        model = ?model,
...
mod tests {
    ...
    /// metrics on senda.network/status stayed permanently empty because
