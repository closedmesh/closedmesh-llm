//! Publish and discover senda meshes via Nostr relays.
//!
//! A running mesh publishes a replaceable event (kind 31990, d-tag "senda")
...
// ---------------------------------------------------------------------------
// Keys — stored in ~/.senda/nostr.nsec
// ---------------------------------------------------------------------------

fn nostr_key_path() -> Result<std::path::PathBuf> {
    let home =
        dirs::home_dir().ok_or_else(|| anyhow::anyhow!("Cannot determine home directory"))?;
    Ok(home.join(".senda").join("nostr.nsec"))
}
...
pub async fn publish_loop(node: crate::mesh::Node, keys: Keys, config: PublishLoopConfig) {
    ...
    loop {
        ...
        // entry.senda.network running `client --auto --publish --mesh-name X`)
...
///   - the blessed community name "senda".
///
...
pub fn is_auto_eligible(mesh: &DiscoveredMesh) -> bool {
    match mesh.listing.name.as_deref() {
        None => true,
        Some(name) => name.eq_ignore_ascii_case("senda"),
    }
}
...
pub fn score_mesh(mesh: &DiscoveredMesh, _now_secs: u64, last_mesh_id: Option<&str>) -> i64 {
    ...
    // anything else in `--auto`. The literal name "senda" is treated as a
    ...
    match mesh.listing.name.as_deref() {
        None => score += 300,
        Some(n) if n.eq_ignore_ascii_case("senda") => score += 300,
        Some(_) => {}
    }
...
/// entry.senda.network is also publishing the same name — it lets the
...
pub fn smart_auto_with_self(
    ...
    // plus the blessed name "senda". Other named meshes are still publicly
...
mod scoring_tests {
    ...
    #[test]
    fn score_senda_alias_matches_unnamed() {
    ...
    fn other_named_mesh_not_auto_eligible() {
        // The blessed community name is `senda` (per `is_auto_eligible`).
        // We exercise case-insensitive matching with `SENDA`, and confirm
        ...
        let community = make_mesh(Some("senda"), Some("c"), &[], 1, 0, 0, 0);
        let community_caps = make_mesh(Some("SENDA"), Some("c2"), &[], 1, 0, 0, 0);
...
mod smart_auto_tests {
    ...
    fn smart_auto_both_community_aliases_eligible() {
        // Both unnamed listings and the "senda" alias are eligible for
        ...
        // candidates and that `"senda"` is no longer penalised relative
        // to unnamed.
        let meshes = vec![
        ...
        assert_eq!(
            ...
            "None and 'senda' should score equally as community aliases",
        );
    ...
    fn smart_auto_excludes_other_named_meshes() {
        ...
        // (unnamed or name == "senda"). Other named meshes — even though
    ...
    fn smart_auto_larger_unnamed_beats_smaller_alias() {
        // Both unnamed and "senda" are eligible with the same name bonus.
    ...
    fn smart_auto_sticky_preference() {
        ...
        let dir = dirs::home_dir().unwrap().join(".senda");
...
mod rotate_key_tests {
    ...
    // rotate_keys uses hardcoded paths (~/.senda/), so we test the logic
...
mod key_file_tests {
    ...
    fn load_or_create_keys_at_round_trips() {
        let path = temp_key_path("senda-nostr-key");
    ...
    fn load_or_create_keys_at_hardens_existing_permissions() {
        ...
        let path = temp_key_path("senda-nostr-key-perms");
    ...
    fn load_or_create_keys_at_rejects_symlink_key() {
        ...
        let path = temp_key_path("senda-nostr-key-symlink");
        let dir = path.parent().unwrap();
        std::fs::create_dir_all(dir).unwrap();
...
mod integration_tests {
    ...
    async fn publish_discover_round_trip() {
        ...
        let mesh_name = format!("senda-test-{}", rand::random::<u32>());
