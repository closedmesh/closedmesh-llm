...
for i in $(seq 1 "$MAX_INFERENCE_ATTEMPTS"); do
    ...
    if [ "$HOST_HTTP_CODE" = "200" ]; then
        HOST_READY=$(echo "$HOST_BODY" | python3 -c "import sys,json; r=json.load(sys.stdin); n=r.get('usage',{}).get('completion_tokens',0) or 0; m=r['choices'][0]['message']; c=(m.get('content') or m.get('reasoning_content') or '').strip(); print(c or (f'<{n} blank tokens>' if n>0 else ''))" 2>/dev/null || echo "")
...
CONTENT2=$(echo "$RESPONSE2" | python3 -c "import sys,json; r=json.load(sys.stdin); n=r.get('usage',{}).get('completion_tokens',0) or 0; m=r['choices'][0]['message']; c=(m.get('content') or m.get('reasoning_content') or '').strip(); print(c or (f'<{n} blank tokens>' if n>0 else ''))" 2>/dev/null || echo "")
if [ -z "$CONTENT2" ]; then
